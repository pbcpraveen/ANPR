...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  5 13:29:11 2019
...
#-----------------------------------------------------------------------------------------
p = cv2.resize(plate1,(len(plate1[0])*5,len(plate1)*5))
p1 = p[:,:,0]
r,p1 = cv2.threshold(p1,30,255,cv2.THRESH_BINARY)
contours,hi = cv2.findContours(p1.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
chars = dict()
for c in contours:        
    x,y,w,h=cv2.boundingRect(c)
    #cv2.rectangle(backup,(x,y),(x+w,y+h),(60,255,40),1)
    char_image=p1[y:y+h,x:x+w]

    if(w*h>0 and w>10 and h>150 and w<500):
        char_image=p1[y:y+h,x:x+w]
        cv2.imshow('img2',char_image)
        cv2.waitKey(0)
        canny_img=cv2.Canny(char_image,30,200)
